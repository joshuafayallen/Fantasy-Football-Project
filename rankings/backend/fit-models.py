...
logos_data = pl.read_csv(
    "https://raw.githubusercontent.com/shoenot/NFL-Team-Logos-Transparent-Squared/refs/heads/main/logo_urls.csv"
...
for i in seasons:
    df = seasons_data.filter(pl.col("season") == i)
    stats = team_stats.filter(
        (pl.col("season") == i)
        & (pl.col("epa").is_not_null())
        & (pl.col("play_type").is_in(["pass", "run"]))
        & (pl.col("down").is_in([1, 2, 3, 4]))
    ).select(pl.col("season", "week", "epa", "play_id", "posteam", "defteam"))
    off_epa = stats.group_by(["posteam"]).agg(
        pl.col("epa").mean().alias("off_epa_per_play")
    )

    def_epa = stats.group_by(["defteam"]).agg(
    ...
    df = pl.from_pandas(
        az.summary(out, var_names=["p_win_neutral", "team_mu"]).reset_index()
    ).with_columns(
        pl.col("index").str.extract(r"\[(\w+)\]", group_index=1).alias("team")
    )
    ability = (
        df.filter(pl.col("index").str.contains(r"team_mu"))
        .rename(
            {
                "mean": "team_ability",
                "hdi_3%": "hdi_low_ability",
                "hdi_97%": "hdi_high_ability",
            }
        )
    ...
    win_prob = df.filter(
        pl.col("index").str.contains(r"p_win_neutral")
    ).rename(
        {
            "mean": "team_win_prob",
            "hdi_3%": "hdi_low_prob",
    ...
    team_lookup = pl.DataFrame(
        {"team_id": range(len(bt_mag_mod.teams)), "team": bt_mag_mod.teams}
    )

    record = (
        wins.join(losses, left_on="winner_id", right_on="loser_id", how="full")
        .with_columns(
            pl.col("winner_id").fill_null(pl.col("loser_id")).alias("team_id")
        )
        .select("team_id", "wins", "losses")
        .join(team_lookup, on="team_id", how="left")
        .with_columns(
            pl.col("wins").fill_null(0), pl.col("losses").fill_null(0)
        )
        ...
        .with_columns(
            ...
            .then(
                pl.concat_str([pl.col("wins"), pl.lit("-"), pl.col("losses")])
            )
            .otherwise(
                pl.concat_str(
                    [
    ...
    skills = skills.join(
        logos_data, left_on=["logo_team"], right_on=["team"], how="full"
    )
    skills = skills.join(
        epa, left_on=["logo_team"], right_on=["posteam"]
    ).select(pl.exclude("logo_team"))
    skills = skills.join(record, on="team").filter(
