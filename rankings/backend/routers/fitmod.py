...
router = APIRouter(prefix="/fit", tags=["model"])
...
def fit_model(request: SeasonRequest):
    ...
    print(
        (
            f"Available keys in precomputed_results:"
            f"{list(precomputed_results.keys())}"
        )
    )
    ...
    wins = bt_mag._cleaned.group_by(["winner_id"]).agg(pl.len().alias("wins"))
    losses = bt_mag._cleaned.group_by(["loser_id"]).agg(
        pl.len().alias("losses")
    )

    team_lookup = pl.DataFrame(
        {"team_id": range(len(bt_mag.teams)), "team": bt_mag.teams}
    )
